...
class BingSearchApproach(Approach):
    def __init__(self, search_client: SearchClient, openai_deployment: str, sourcepage_field: str, content_field: str, bing_subscription_key: str, bing_search_endpoint: str, azure_openai_key: str, azure_openai_base: str):
    ...
    def get_bing_search_result(self, question, top):
        print(f"Searching Bing for {question}")
        mkt = 'zh-CN'
    ...
    def search(self, q: str, overrides: dict) -> str:
        ...
        r = self.get_bing_search_result(q, top=top)
    ...
    def lookup(self, q: str) -> str:
        r = self.get_bing_search_result(q, top=1)
            
        answers = r.get_answers()
        if answers and len(answers) > 0:
            return answers[0].text
        if r.get_count() > 0:
            return "\n".join(d['snippet'] for d in r)
        return None        
    ...
    def run(self, q: str, overrides: dict) -> any:
        ...
        # llm = AzureChatOpenAI(openai_api_base=self.openai_deployment, temperature=overrides.get("temperature") or 0.3, openai_api_key=openai.api_key)
        ...
        tools = [
            Tool(name="Search", description="Search Bing", func=lambda q: self.search(q, overrides)),
            Tool(name="Lookup", description="Lookup Bing",func=self.lookup)
        ]
...
"Actions can be in any format, but must be in the format of the example actions. " \
"Translate the Finish action into Chinese before answering the question. Do not use the Finish action as an observation. " \
"Do not translate Finish into Chinese. " 